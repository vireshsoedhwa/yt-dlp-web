...
│   │   ├── core/
│   │   │   ├── config.py           # Settings (download dir, Redis URL, queue, archive, session)
│   │   │   ├── queue.py            # Redis + Queue + dedup + session-to-file mapping
│   │   │   ├── files_service.py    # Hard link management, serve-then-delete cleanup
│   │   │   └── yt_dlp_service.py   # yt-dlp wrapper + archive + parse_video_id + self-update
│   │   └── models/
...
- `GET  /`                      — health check
- `POST /api/info`              — `{ "url": "..." }` -> video metadata (synchronous)
- `POST /api/download`         — `{ "url": "...", "format": "...", "audio_only": false }` -> `{ "job_id": "...", "status": "queued" }`
...
- `POST /api/update`            — self-upgrade yt-dlp to latest PyPI version (fixes 403 errors without rebuild)
...
All `/api/files`, `/api/files/{filename}`, `/api/purge`, and `/api/purge/preview` endpoints require an `X-Session-ID` header for session isolation.
...
- Session isolation: frontend generates a UUID stored in localStorage, sent as `X-Session-ID` header. Backend uses it as a namespace key to isolate files per session
...
- Fallback purge: files not claimed within 3 hours are purged via `POST /api/purge` (also cleans up orphaned originals and archive entries)
